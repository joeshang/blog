...
* 对于不同类型的进程，Chrome 启动的程序不一样，可以分析出来 Chrome 子进程的类型
  ...
  * Utility 是 Google Chrome Helper，type 为 render，会有一个 `utility-sub-type` 指定 Utility 的具体功能
...
### 确定方案稳定性
这个方案虽然可行，且比较 tricky，但是否稳定？让源码为我们证明！
...
#### frame.processId
对于 frame 中的 processId，通过[源码](https://source.chromium.org/chromium/chromium/src/+/main:out/Debug/gen/content/browser/resources/process/tsc/process_internals.mojom-webui.js?q=processId&ss=chromium&start=41)可以看到，JS binding 调用的是 ProcessInternalsHandler 中的 `frame_info->process_id = frame->GetProcess()->GetID();`

其中 frame 为 RenderFrameHostImpl，其 GetProcess 返回的是 RenderProcessHost，实际上由 `agent_scheduling_group_`  返回
...
解决了 Chrome，由于 Edge 同样基于 Chromium，因此 Edge 可以直接加载 Chrome 的 Extension，另外，Edge 每个 Tab 的子进程的启动参数中也有 renderer-cliend-id，所以，Edge 的方案与 Chrome 基本一致：
...
3. mainfest 中 name 必须为 小写字母 . 和 _，其他均为非法
4. mainfest 应该放到 `~/Library/Application Support/Google/Chrome/NativeMessagingHosts` 目录
5. mainfest 的名字应该与 name 一致，为 name.json
